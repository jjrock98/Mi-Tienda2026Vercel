              ...
              {metodo === 'transferencia' && bankInfo && (
                <div className="mt-4 rounded-xl bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800 p-4 space-y-3">
                  <h4 className="font-semibold text-sm text-blue-800 dark:text-blue-400 flex items-center gap-2">
                    <Building2 size={16} /> Datos para la transferencia
                  </h4>
                  <div className="space-y-1.5 text-sm">
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs break-all">{bankInfo.cbu || '—'}</span>
                        {bankInfo.cbu && (
                          <button onClick={() => copyToClipboard(bankInfo.cbu!, 'CBU')} className="text-blue-500 hover:text-blue-700">
                            <Copy size={14} />
                          </button>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs">{bankInfo.alias || '—'}</span>
                        {bankInfo.alias && (
                          <button onClick={() => copyToClipboard(bankInfo.alias!, 'Alias')} className="text-blue-500 hover:text-blue-700">
                            <Copy size={14} />
                          </button>
                        )}
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="font-medium">{bankInfo.tipo_cuenta || '—'}</span>
                    </div>
                    <div className="flex items-center justify-between gap-2">
                    ...
                    {bankInfo.instrucciones && (
                      <div className="mt-2 pt-2 border-t border-blue-100 dark:border-blue-800 text-xs text-blue-700 dark:text-blue-300">
                        <p><span className="text-muted">Instrucciones:</span> {bankInfo.instrucciones}</p>
                      </div>
                    )}
                  </div>
                  <div className="mt-2 rounded-lg bg-blue-100/50 dark:bg-blue-900/30 p-2 text-xs text-blue-700 dark:text-blue-300 text-center">
                    Después de transferir, subí el comprobante desde la página de confirmación.
                  </div>
                </div>
              )}

              {metodo === 'transferencia' && !bankInfo && (
                <div className="mt-4 rounded-xl bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 p-4 text-sm text-amber-700 dark:text-amber-400 flex items-start gap-3">
                  <AlertCircle size={18} className="shrink-0 mt-0.5 text-amber-600" />
                  <div>
                    <strong>No hay datos bancarios configurados.</strong>
                    <p className="mt-1 text-xs">El administrador debe configurar los datos bancarios en el panel de control.</p>
                  </div>
                </div>
              )}

              {metodo === 'transferencia' && (
                <div className="mt-2 rounded-xl bg-green-50 dark:bg-green-950/20 p-4 text-sm text-green-700 dark:text-green-400 flex items-start gap-3">
